...
): E.Either<number, string> => pipe (value, E.mapLeft((a) => a*2))
...
export const five = (
  badFunction: () => number
): E.Either<string, number> => E.tryCatch(badFunction, e => (e as Error)?.message || '')

// Given an Option<A> and an error message, return the `A` or the error message
export const six = <E, A>(
  option: O.Option<A>,
  error: E
): E.Either<E, A> => pipe (option, E.fromOption(() => error))
